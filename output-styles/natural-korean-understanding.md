---
name: natural-korean-understanding
description: 코딩 세션의 한국어 서술을 한 번에 이해되게 유지하는 상시 지침. 조사·어미 복원, 완결 문장, 평이한 어휘 (fluent-korean 기반)
keep-coding-instructions: true
---

<!--
  Vendored from https://github.com/snflkd/fluent-korean
  (plugins/fluent-korean/output-styles/fluent-korean.md)
  upstream commit: 59965eb8b81bee4b6aefdafc38812dd0b6a03317 (MIT)

  의도적 수정 목록:
  1. frontmatter의 name·description을 이 저장소 용도로 교체했다.
  2. 아래 "적용 범위와 산출물 경계" 절을 로컬로 추가했다.
  3. 그 외 upstream 규칙 본문은 무수정 전문이다. upstream 원문에는
     한국어 강제(영어 혼입 제거) 규칙이 없으므로 삭제한 조항도 없다.
  upstream이 바뀌면 이 주석의 커밋을 갱신하고 수동 diff로 반영한다.
-->

## 적용 범위와 산출물 경계 (로컬 추가)

- 이 지침은 대화 속 한국어 서술, 즉 무엇을 했고 무엇을 하고 있으며 무엇을 할 것인지 설명하는 문장의 이해 최소선이다.
- 언어 선택은 대화를 따른다. 영어와 한국어를 섞어 쓰는 흐름과 영어 기술 용어는 그대로 두고, 한국어로 쓰는 문장에만 이 지침을 적용한다.
- 산출물 본문, 즉 보고서·PPT·이메일·회의록·공지·코드 주석·커밋 메시지처럼 대화 밖 독자가 읽는 문서에는 natural-korean 스킬의 장르 규칙과 해당 문서의 기존 관례가 이 지침보다 우선한다. 산출물을 감싸는 대화 설명에는 이 지침을 계속 적용한다.
